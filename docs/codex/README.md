# Codex Notes

This directory contains design and planning notes produced during repository analysis and implementation planning.

## Documents

- [UDP Load Balancing and HTTP/3 Roadmap](udp-http3-roadmap.md)
- [UDP and HTTP/3 Early Milestones Task Breakdown](udp-http3-m0-m1-tasks.md)
- [UDP and HTTP/3 Later Milestones Task Breakdown](udp-http3-m2-m7-tasks.md)
- [UDP, QUIC, and HTTP/3 Cycle 2 Roadmap](udp-http3-cycle-2-roadmap.md)
- [UDP, QUIC, and HTTP/3 Cycle 2 Task Breakdown](udp-http3-cycle-2-tasks.md)
- [UDP Transport Boundary Design Note](udp-transport-boundary.md)
- [UDP Balancing Model Design Note](udp-balancing-model.md)
- [UDP Upstream Model Design Note](udp-upstream-model.md)
- [QUIC Integration Strategy Design Note](quic-integration-strategy.md)
- [UDP and HTTP/3 Validation Notes](udp-http3-validation.md)
